...
struct DeviceCapabilities {
    const int max_workgroup_size;
    //const int optimal_block_size; 

    explicit DeviceCapabilities(const sycl::queue& queue) {
        auto device = queue.get_device();
        max_workgroup_size = device.get_info<sycl::info::device::max_work_group_size>();
        //optimal_block_size = ;

    }

    // Getters here.
    int GetMaxWorkgroupSize() const { return max_workgroup_size; }
    //int GetOptimalBlockSize() const { return optimal_block_size; }
};
